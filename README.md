...
| `proctor_policy` | Operator safety plumbing: a fail-closed policy gate (allow / block / sensitive by bundle id, with TTL-bounded approval tokens) and a redacting audit trail that stores typed values as length + SHA-256, never in the clear. The trail is sealed at rest, and each entry is chained to the one before it and signed by a key held in this Mac's secure element, so an entry added, removed, edited or rolled back anywhere else is reported rather than read as genuine. `auditVerdict` on a status call says whether it is the trail Proctor wrote. |
