...
134 piece(s) of work remain — 4 product, 26 evidence, 103 decision — across 925 ledger rows. This reckoning speaks for 463/466 (99%) of the campaign's designed cases and 124/141 (88%) of its stated requirements; the rest is not known to be done, it is simply not known.

## What it can speak for

| Axis | Measured | Of | % | What the number means |
|---|---:|---:|---:|---|
| Cases adjudicated | 463 | 466 | 99.4% | an instrument returned a verdict on the product — pass or fail. A fail is knowledge; this is not a pass rate. |
| Cases ruled out by decision | 0 | 466 | 0.0% | somebody ruled the cell out of scope or not applicable. A decision, not a measurement, and it is kept out of the line above on purpose. |
...
| `unmeasured` | 23 + 3 blockers | 26 | evidence-work | nobody found out — the work here is becoming able to tell |
...
| `verified-done` | 0 | 791 | none | not remaining; kept so the denominator is honest |
...
|---|---:|---:|---|
| `BLOCK-0001` | 1 | +1/466 (+0.2 pts) | inconclusive: PersonInput.isAPerson requires sourcePid == 0, which only hardware carries and no second process can forge, so the human-input path REQ- |
| `BLOCK-0002` | 1 | +1/466 (+0.2 pts) | inconclusive: Proctor never observes the driver's cursor, so no instrument on this lane can read whether that cursor is over a covered target. The rea |
| `BLOCK-0003` | 1 | +1/466 (+0.2 pts) | inconclusive: the runs the report's first clause describes are driven by another automation stack entirely, so there is no Proctor run to instrument.  |
