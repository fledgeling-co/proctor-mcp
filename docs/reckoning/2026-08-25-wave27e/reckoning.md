...
11 piece(s) of work remain — 1 product, 4 evidence, 0 decision — across 962 ledger rows. This reckoning speaks for 468/471 (99%) of the campaign's designed cases and 139/141 (99%) of its stated requirements; the rest is not known to be done, it is simply not known.
...
| `retirable` | 6 | 6 | bookkeeping | already done to a standard that can carry the claim — close it |
| `waived` | 0 | 139 | exception | somebody decided not to — an exception, and it stays visible |
| `verified-done` | 0 | 812 | none | not remaining; kept so the denominator is honest |
...
- **BRIEF-47-supervision-survives-delegation** — Supervision survives delegation
  - observed at rung 'effect-witness', at or above the 'outcome' floor, on a cited join — the work this brief asks for appears already done
- **BRIEF-68-the-operator-cli** — `proctor`, the operator CLI
  - observed at rung 'effect-witness', at or above the 'outcome' floor, on a cited join — the work this brief asks for appears already done
- **BRIEF-72-tests-that-mutate-and-never-read-back** — Seventy-eight tests that change state and never look at it
  - observed at rung 'effect-witness', at or above the 'outcome' floor, on a cited join — the work this brief asks for appears already done
