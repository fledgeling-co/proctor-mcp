...
150 piece(s) of work remain — 24 product, 39 evidence, 87 decision — across 796 ledger rows. This reckoning speaks for 99% of the campaign's designed cases and 77% of its stated requirements; the rest is not known to be done, it is simply not known. 1 input(s) could not be classified — id-shaped token 'CASE-9999' on 1 row(s) — and each is named with its row count rather than counted as work or as done. 79 brief(s) could not be tied to the registry at all; they are listed as `unjoined` and counted as decision work, rather than assumed unbuilt.

## What it can speak for

| Axis | Measured | Of | % | What the number means |
|---|---:|---:|---:|---|
| Cases adjudicated | 396 | 400 | 99.0% | an instrument returned a verdict on the product — pass or fail. A fail is knowledge; this is not a pass rate. |
| Cases ruled out by decision | 0 | 400 | 0.0% | somebody ruled the cell out of scope or not applicable. A decision, not a measurement, and it is kept out of the line above on purpose. |
| Requirements observed | 89 | 116 | 76.7% | somebody watched it happen, rather than the project reporting it of itself. |
| Surfaces spoken for | 27 | 27 | 100.0% | at least one case on this surface reached a verdict. |
| Briefs joined to evidence | 21 | 100 | 21.0% | the brief could be tied to something in the registry at all. |
...
|---|---:|---:|---|---|
| `unjoined` | 79 | 79 | decision-work | named in a brief; the join reached nothing, so its state is unknown either way |
| `broken` | 24 | 24 | product-work | measured, and the answer was no |
| `unmeasured` | 35 + 4 blockers | 39 | evidence-work | nobody found out — the work here is becoming able to tell |
| `undecided` | 8 | 8 | decision-work | the documents and the evidence disagree; needs a person |
| `verified-done` | 0 | 646 | none | not remaining; kept so the denominator is honest |
...
|---|---:|---:|---|
| `BLOCK-0001` | 1 | +0.2 pts | INCONCLUSIVE, AND THE INSTRUMENT IS NAMED. REQ-024 declares effect `subprocess` and the census names Process() in Actuation/CuaClients.swift as its pr |
| `BLOCK-0002` | 1 | +0.2 pts | inconclusive: PersonInput.isAPerson requires sourcePid == 0, which only hardware carries and no second process can forge, so the human-input path REQ- |
| `BLOCK-0003` | 1 | +0.2 pts | inconclusive: Proctor never observes the driver's cursor, so no instrument on this lane can read whether that cursor is over a covered target. The rea |
| `BLOCK-0004` | 1 | +0.2 pts | inconclusive: the runs the report's first clause describes are driven by another automation stack entirely, so there is no Proctor run to instrument.  |
...
- **BRIEF-90-the-four-nobody-owns** — Twenty-seven unwrapped tests, a fixed timer, and two witnesses the rung wants
  - the registry records a defect or a failing case against this brief's subject
- **BRIEF-98-a-version-string-is-not-the-artifact** — A version string is not the artifact
  - the registry records a defect or a failing case against this brief's subject
...
- **DEF-141** — REQ-055's original sentence certified reads, the whole run and every operator path; the witness watches writes, two calls and one root
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-204** — Two different tools with the same version string compare as the same tool
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-215** — Four ledger rows have no spec file, so two briefs have no artifact that could cite them
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-216** — The installed reckon plugin is still 1.0.0, and 1.0.0 crashes on this registry
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-220** — sweepK-scaling.json lists two frames its own mode table has no capture of (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-223** — A file's surface prefix binds it to a surface the picture does not show (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-230** — A requirement carries an evidence word its own schema rejects, and the tool explained it as a self-report
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
...
- **DEF-180** — A Screen Recording answer is frozen for the life of the agent process, so a revoked grant is reported as granted until it restarts
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-218** — Nine files named for nine engine surfaces are app-icon renders (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-219** — Three files named for the takeover shield contain no image at all (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-221** — One image carries three captions across two unrelated sweeps, and two wedged timestamps share one frame (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-222** — sweepL-status-agent-down.png depicts a Ready window, and the two agent-down frames are one image (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-224** — Case evidence and the lineage manifest disagree about the same five files (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-225** — A passing case cites a shots file that does not exist, and no gate looks (recorded)
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-228** — LEDGER.md is the ledger of record and no standing instrument reads it
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-229** — capture-lineage.py never reads cases.json either, so DEF-227's class reaches the gate
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
...
- **BRIEF-00-WAVE-7-DIRECTION** — Wave 7 direction: Cua underneath, Proctor on top
  ...
  - nearest the join considered: REQ-072 (0.10), REQ-055 (0.09), REQ-094 (0.09)
- **BRIEF-01-cua-schema-facade** — Stock computer-use schema façade (Anthropic + OpenAI)
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.08), DEF-115 (0.05), REQ-055 (0.05)
...
- **BRIEF-03-menu-bar-key-equivalents** — Menu-bar enumeration with key-equivalents
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.06), REQ-026 (0.05), REQ-108 (0.05)
- **BRIEF-04-app-scripting-dictionary** — App scripting-dictionary introspection
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.07), REQ-098 (0.05), REQ-097 (0.05)
- **BRIEF-05-audit-trail-policy-gate** — Redacting audit trail + policy / approval gate
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-055 (0.07), REQ-130 (0.06), REQ-098 (0.06)
- **BRIEF-06-vision-capture-normalisation** — Vision-capture normalisation + reported scale factor
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: DEF-025 (0.05), REQ-005 (0.04), REQ-072 (0.04)
...
- **BRIEF-08-mcp-surface-modernization** — MCP surface modernization
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-055 (0.07), REQ-108 (0.07), REQ-096 (0.06)
- **BRIEF-09-process-kill-fs-jail** — Process kill + filesystem jail
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-055 (0.10), REQ-085 (0.07), REQ-072 (0.07)
- **BRIEF-10-pointer-overlay-captures** — Pointer / target overlay in captures
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.07), DEF-115 (0.06), REQ-112 (0.06)
- **BRIEF-11-stability-per-step-pointer** — Pointer marker in proctor_stability per-step artifacts
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.07), REQ-055 (0.07), REQ-086 (0.06)
...
- **BRIEF-25-second-browser-lane-for-obscuras-limits** — A second browser lane for what Obscura cannot do
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-085 (0.08), REQ-094 (0.07), REQ-072 (0.07)
- **BRIEF-26-prefer-background-and-pointer-in-plane** — Prefer the background, and draw the pointer where the work is happening
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.10), REQ-055 (0.08), REQ-094 (0.08)
...
- **BRIEF-28-menu-bar-character-when-idle** — The menu bar shows the character when idle, not a status symbol
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.08), REQ-055 (0.07), REQ-094 (0.07)
- **BRIEF-29-re-check-now-says-what-it-checks** — "Re-check now" does not say what it checks
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.10), REQ-055 (0.09), REQ-121 (0.09)
- **BRIEF-30-a-home-for-the-proctor-switches** — A home for the PROCTOR_* switches
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-094 (0.10), REQ-072 (0.10), REQ-055 (0.09)
...
- **BRIEF-33-the-audit-trail-is-signed** — The audit trail is signed, and it records what Proctor recommended
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.08), REQ-094 (0.07), REQ-085 (0.07)
...
## Inputs this reckoning could not classify (1)
...
|---|---|---:|---|---|
| id-shaped token | `CASE-9999` | 1 | `unmeasured` | BRIEF-97-an-input-the-check-cannot-classify |
