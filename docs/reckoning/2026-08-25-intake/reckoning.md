...
9 piece(s) of work remain — 1 product, 2 evidence, 6 decision — across 995 ledger rows. This reckoning speaks for 492/495 (99%) of the campaign's designed cases and 139/141 (99%) of its stated requirements; the rest is not known to be done, it is simply not known.

## What it can speak for

| Axis | Measured | Of | % | What the number means |
|---|---:|---:|---:|---|
| Cases adjudicated | 492 | 495 | 99.4% | an instrument returned a verdict on the product — pass or fail. A fail is knowledge; this is not a pass rate. |
| Cases ruled out by decision | 3 | 495 | 0.6% | somebody ruled the cell out of scope or not applicable. A decision, not a measurement, and it is kept out of the line above on purpose. |
...
| `undecided` | 6 | 6 | decision-work | the documents and the evidence disagree; needs a person |
| `waived` | 0 | 147 | exception | somebody decided not to — an exception, and it stays visible |
| `verified-done` | 0 | 839 | none | not remaining; kept so the denominator is honest |
...
- **BRIEF-145-an-arming-claim-nobody-checks** — An arming claim nobody checks
...
- **BRIEF-146-a-gate-that-reads-the-number-and-not-the-word** — A gate that reads the number and not the word
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent — route to spec-validation before retiring
- **BRIEF-147-a-gate-that-loses-which-check-failed** — A gate that loses which check failed
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent — route to spec-validation before retiring
- **BRIEF-148-nothing-counts-the-ledgers-status-words** — Nothing counts the ledger's status words
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent — route to spec-validation before retiring
- **BRIEF-149-a-capture-store-an-outside-probe-can-find** — A capture store an outside probe can find
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent — route to spec-validation before retiring
- **BRIEF-150-an-auditor-clearing-its-own-gate** — An auditor clearing its own gate
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent — route to spec-validation before retiring

## Decisions on the record (147)
