...
138 piece(s) of work remain — 9 product, 37 evidence, 92 decision — across 839 ledger rows. This reckoning speaks for 422/426 (99%) of the campaign's designed cases and 98/125 (78%) of its stated requirements; the rest is not known to be done, it is simply not known. 76 brief(s) could not be tied to the registry at all; they are listed as `unjoined` and counted as decision work, rather than assumed unbuilt.

## What it can speak for

| Axis | Measured | Of | % | What the number means |
|---|---:|---:|---:|---|
| Cases adjudicated | 422 | 426 | 99.1% | an instrument returned a verdict on the product — pass or fail. A fail is knowledge; this is not a pass rate. |
| Cases ruled out by decision | 0 | 426 | 0.0% | somebody ruled the cell out of scope or not applicable. A decision, not a measurement, and it is kept out of the line above on purpose. |
| Requirements observed | 98 | 125 | 78.4% | somebody watched it happen, rather than the project reporting it of itself. |
| Surfaces spoken for | 30 | 30 | 100.0% | at least one case on this surface reached a verdict. |
| Briefs joined to evidence | 26 | 102 | 25.5% | the brief could be tied to something in the registry at all. |
...
|---|---:|---:|---|---|
| `unjoined` | 76 | 76 | decision-work | named in a brief; the join reached nothing, so its state is unknown either way |
| `broken` | 9 | 9 | product-work | measured, and the answer was no |
| `unmeasured` | 33 + 4 blockers | 37 | evidence-work | nobody found out — the work here is becoming able to tell |
| `undecided` | 16 | 16 | decision-work | the documents and the evidence disagree; needs a person |
| `verified-done` | 0 | 701 | none | not remaining; kept so the denominator is honest |
...
|---|---:|---:|---|
| `BLOCK-0001` | 1 | +1/426 (+0.2 pts) | INCONCLUSIVE, AND THE INSTRUMENT IS NAMED. REQ-024 declares effect `subprocess` and the census names Process() in Actuation/CuaClients.swift as its pr |
| `BLOCK-0002` | 1 | +1/426 (+0.2 pts) | inconclusive: PersonInput.isAPerson requires sourcePid == 0, which only hardware carries and no second process can forge, so the human-input path REQ- |
| `BLOCK-0003` | 1 | +1/426 (+0.2 pts) | inconclusive: Proctor never observes the driver's cursor, so no instrument on this lane can read whether that cursor is over a covered target. The rea |
| `BLOCK-0004` | 1 | +1/426 (+0.2 pts) | inconclusive: the runs the report's first clause describes are driven by another automation stack entirely, so there is no Proctor run to instrument.  |
...
- **BRIEF-78-the-skill-and-the-guest-lane** — The skill advertises 20 tools of 27, and talks an agent out of the guest lane
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent; and the join as a whole is too weak to carry a retirement claim — route to spec-validation before retiring
...
- **BRIEF-91-eight-more-operator-paths-with-no-seam** — Eight more operator paths with no seam
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent; and the join as a whole is too weak to carry a retirement claim — route to spec-validation before retiring
- **BRIEF-92-a-spec-says-which-brief-it-came-from** — A spec says which brief it came from
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent; and the join as a whole is too weak to carry a retirement claim — route to spec-validation before retiring
- **BRIEF-94-a-reckoning-worth-comparing-against** — A reckoning worth comparing against
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent; and the join as a whole is too weak to carry a retirement claim — route to spec-validation before retiring
- **BRIEF-96-what-1-1-0-still-groups-and-still-grades** — What 1.1.0 still groups, and what it still lets a citation grade
  - looks done, but the strongest oracle behind it is 'none', below the 'outcome' floor for retiring intent; and the join as a whole is too weak to carry a retirement claim — route to spec-validation before retiring
