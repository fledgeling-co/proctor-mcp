...
Deliver both halves at the scope below. Guest **creation and provisioning stay out**: making a
VM, granting Accessibility and Screen Recording inside one, and cloning the granted result are
things a person does with the provider's own CLI, and the grant-once-then-clone recipe stays
documentation. If a better approach than the one described here becomes apparent, say so in a
sentence and build what is specified rather than substituting it.
...
5. **A5** — `RunLane` gains a counted lane alongside its two mutex lanes, and the counted lane's
   ...
   is Apple's rule about macOS guests and not a property of virtualisation.
