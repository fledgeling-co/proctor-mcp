...
| **Measured gate** | 1,558 tests in 185 suites, green (`swift test`, 2026-08-20) |
...
Nine switches have a home in the status window (`SwitchCatalogue`). Drawing switches default **on**;
...
| `PROCTOR_OVERLAY_CAPTURE` | capability | next start | Drop the run panel's, the takeover tint's and the drawn pointer's exclusion from screen capture, so a test can photograph what they draw. While it is on, anything recording the screen sees them too. Second press required. |
...
|---|---|---|
| Unit and integration suite | `swift test` green, no failures, no timeouts | **1,558 tests in 185 suites**, green twice, 2026-08-20 |
...
| UI test campaign | Every armed case passes or is `n/a` with a structural reason | **32 pass, 0 n/a of 32**, all 32 armed by a recorded mutation, 2026-08-20 |
...
CASE-0008 (run HUD raster) and CASE-0010 (takeover shield raster) were `n/a` for a real reason and
are now passes, which is worth stating precisely because the wrong lesson is available. No capture
channel can photograph a `sharingType = .none` window: ScreenCaptureKit delivers no frame,
`screencapture -l` exits 1 with "could not create image from window", a region capture of the rect
returns what is behind the panel, and the CoreGraphics image APIs are obsoleted in macOS 15. That
exclusion is deliberate, and it stays the default. `PROCTOR_OVERLAY_CAPTURE` (section 18) lifts it
for a test run only, off unless somebody sets it, and the default's refusal is what arms both cases.
Neither may be restored to `pass` on a design sprite or a transparent frame, which is what the
earlier pass rested on.

Every arming claim in the campaign is now a recorded mutation rather than a sentence: the behaviour
was broken in the source, the named suite was watched to go red, and it was put back. Nine of those
mutations initially changed nothing, which is how the suite's real holes were found; see DEF-006 and
`docs/test-campaign/evidence/arming-mutations.json`.
...
while only the menu-bar extras item is up, so reopen activated a process showing nothing. Alongside
it: `OverlayCapture.swift` and the `PROCTOR_OVERLAY_CAPTURE` switch; `CaptureTrust.swift`, which
lifts the frame-trustworthiness and settle-signal decisions out of the agent so they can be tested at
all; a bounded poll in `AgentModel` so a wedged agent stops reading as Ready; the drawn pointer's
capture exclusion, which it had never had; and a staleness check in `scripts/install.sh`, which
until now reused an already-notarised bundle even when the source had moved and reported the result
as a successful install. Also uncommitted: `docs/test-campaign/`, `scripts/campaign/`,
`design/surfaces/` and the `vendor/fledgeling-plugins` submodule.
`scripts/build_test_campaign.py` is retired in place and refuses to run.
