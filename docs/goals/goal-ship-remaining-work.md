...
| F-002 | PRO-0161 — Raise or Record Every Case Below the Effect Rung. | `rung_routing.py --gate` | **merged 2026-08-27.** All 43 cases below the effect rung routed: 30 structural, 13 raisable, 0 undecided. The routing verdict is recorded onto cases.json. |
| F-003 | PRO-0162 — Cut or Record Every Durable Boundary. | `journey_census.py --gate` | **merged 2026-08-27.** `journey_census.py` derives cuts from cases and ratchets asserted-only cuts at 43. 44 of 50 boundaries claimed cut across 10 journeys. |
| F-004 | DEF-221 — byte-identical capture shares. | `capture_manifest.py --gate` | **closed 2026-08-27.** What was wrong was the recording, not the frames: the manifest printed the shares and said "identity, not disposition", leaving the disposition to a reader who never arrived. `shared-frames.json` declares all five with why and who decided, and the gate refuses an undeclared one. |
| F-005 | DEF-339 — the Maestro lane's outbound connections. | `finish-line` | **closed 2026-08-27 on the route this brief named.** CASE-0841 reproduces the reading exactly over three runs — 2 peers bare, 1 with the variable set. The remaining peer cannot be stopped without rewriting the operator's config, which PRO-0023 forbids, so it is a recorded ceiling carried in the product's own disclosure rather than an unrepaired half. |
| F-006 | DEF-340 — cases with no lane, and lanes with no case. | `lane_census.py --gate` | **closed 2026-08-27, both halves.** 518 of 518 carry a lane by written rules, 0 unplaceable, all seven declared lanes carry cases. A case's own words beat the rule — six cli cases had been placed on headless by plane. And macos-glass carried 16 cases and a proved artifact while campaign.json never declared it. |
