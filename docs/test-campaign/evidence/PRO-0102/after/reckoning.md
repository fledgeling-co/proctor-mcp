...
132 piece(s) of work remain — 13 product, 36 evidence, 83 decision — across 569 ledger rows. This reckoning speaks for 98% of the campaign's designed cases and 68% of its stated requirements; the rest is not known to be done, it is simply not known. 75 brief(s) could not be tied to the registry at all; they are listed as `unjoined` and counted as decision work, rather than assumed unbuilt.
...
|---|---:|---:|---|---|
| `unjoined` | 75 | 75 | decision-work | named in a brief; the join reached nothing, so its state is unknown either way |
| `broken` | 13 | 13 | product-work | measured, and the answer was no |
...
| `verified-done` | 0 | 437 | none | not remaining; kept so the denominator is honest |
...
- **DEF-151** — Whether the userInput yield fires for a real hand on a real keyboard is still unproved
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-140** — Two force-unwrap shapes sit outside `grep -rn ')!' Tests` and carry DEF-136's hazard unswept
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-141** — REQ-055's original sentence certified reads, the whole run and every operator path; the witness watches writes, two calls and one root
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-162** — The design of record's permissions pane draws no Skip setup, and the build draws one on every step but connect
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-165** — BrowserLaneWiringTests asserts doctor's `ready` is invariant while reading two live machine probes
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-193** — A fifth stale tool count survives in the proctor skill's gemini.md
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-163** — The disabled primary still draws filled accent while the design of record draws it plain, so a control that cannot be pressed looks pressabl
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-180** — A Screen Recording answer is frozen for the life of the agent process, so a revoked grant is reported as granted until it restarts
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-175** — HoldAttributionWiringTests reads the scheduler's ticket before the hold has reached it
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
...
- **BRIEF-00-WAVE-7-DIRECTION** — Wave 7 direction: Cua underneath, Proctor on top
  ...
  - nearest the join considered: REQ-055 (0.09), REQ-085 (0.08), REQ-072 (0.07)
...
- **BRIEF-02-set-of-marks-captures** — Set-of-marks annotated captures
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-083 (0.04), DEF-112 (0.04), DEF-028 (0.04)
...
- **BRIEF-04-app-scripting-dictionary** — App scripting-dictionary introspection
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-072 (0.04), REQ-082 (0.04), DEF-115 (0.04)
- **BRIEF-05-audit-trail-policy-gate** — Redacting audit trail + policy / approval gate
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-055 (0.07), REQ-083 (0.06), REQ-092 (0.05)
...
- **BRIEF-08-mcp-surface-modernization** — MCP surface modernization
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-055 (0.07), REQ-001 (0.05), REQ-088 (0.05)
...
- **BRIEF-13-audit-log-encryption-at-rest** — Encryption-at-rest for the JSONL audit log
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-067 (0.06), REQ-085 (0.06), REQ-091 (0.06)
...
- **BRIEF-28-menu-bar-character-when-idle** — The menu bar shows the character when idle, not a status symbol
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-055 (0.07), REQ-082 (0.07), REQ-085 (0.07)
...
- **BRIEF-39-stability-knows-when-it-is-scoring-a-page** — Stability knows when it is scoring a page
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-085 (0.09), REQ-090 (0.08), REQ-024 (0.07)
- **BRIEF-40-page-scoped-refusal** — Page-scoped refusal
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-085 (0.11), REQ-083 (0.08), REQ-055 (0.07)
