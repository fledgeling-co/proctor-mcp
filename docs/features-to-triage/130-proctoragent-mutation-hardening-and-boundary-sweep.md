...
- audience: Quality engineers maintaining high-assurance mutation kill rates across agent modules
- platforms: mac
- proposed-by-ai: false

## What and why
Mutation analysis in the agent component identified surviving mutations where altered logic did not trigger test failures. A surviving mutant represents an untested branch, decorative assertion, or unverified error condition in the agent codebase. Eliminating surviving mutants ensures every security boundary, permission check, and state transition is actively guarded by assertions that fail when modified.
